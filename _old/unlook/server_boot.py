...
def import_server_module():
    """
    Importa i moduli server in maniera sicura per evitare cicli di importazione.
    Utilizza importazioni dirette e specifiche invece di importare dal pacchetto principale.
    """
    try:
        # Importazione diretta dei moduli necessari
        sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "unlook"))

        # Importa il modulo scanner.py e solo quello che serve
        from unlook.server.scanner import UnlookServer
        from unlook.server.projector.dlp342x import DLPC342XController, OperatingMode, Color, BorderEnable

        return UnlookServer, OperatingMode, Color, BorderEnable
...
def main():
    ...
    try:
        ...
        UnlookServer, OperatingMode, Color, BorderEnable = import_server_module()
